...
-(BOOL) UnzipFileTo:(NSString*) path overWrite:(BOOL) overwrite
{
	...
	do{
		@autoreleasepool {
			...
			dc.month = fileInfo.tmu_date.tm_mon+1; // ZIP month is 0-based!
			...
			do
			{
				...
				if (read >= 0)
				{
					if (fp == NULL) {
						...
						{
							// ============ BEGIN CHANGES TIGERLINE ==================
							NSDictionary* attributes = [_fileManager attributesOfItemAtPath:fullPath error:nil];
							NSDate* fileDate = [attributes fileModificationDate];
							BOOL extistingFileIsOlder = [fileDate timeIntervalSinceDate:orgDate] < 0;
							if( !extistingFileIsOlder || ![self OverWrite:fullPath])
							{
								skipped++;
...
#ifdef DEBUG
	NSLog(@"Unzipping DONE, Skipped %i files", skipped);
#endif
